...
**`OneMatchNoEffectState`**: One scenario is activated targeting `JDBC_STATEMENT_EXECUTE` with `NamePattern.any()`. The scenario matches every call. However, the `ActivationPolicy` is configured with `applicationProbability = 0.0`, which means the scenario is structurally active but the effect never fires (the policy's probability check short-circuits the effect pipeline). The scenario contribution is present in the match result but produces zero terminal action. This measures the cost of a full selector match plus the no-terminal-action decision path.

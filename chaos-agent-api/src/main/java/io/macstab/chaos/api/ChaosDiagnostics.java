 ...
 * JMX at {@code io.macstab.chaos:type=ChaosDiagnostics} and drives
 * {@link io.macstab.chaos.bootstrap.jfr.ChaosStressorSnapshotEvent JFR periodic snapshots}.
 *
...
public interface ChaosDiagnostics {
  ...
  enum ScenarioState {
    ...
    /**
     * The scenario was registered but has not been started yet. Matches no operations.
     * Occurs when using {@link ActivationPolicy.StartMode#MANUAL} before
     * {@link ChaosActivationHandle#start()} is called.
     */
    REGISTERED,

    /**
     * The scenario is running and will evaluate its selector against matching operations.
     * Effects fire when selector matches and {@link ActivationPolicy} constraints pass.
     */
    ACTIVE,
    ...
    /**
     * The scenario was explicitly stopped via {@link ChaosActivationHandle#stop()} or
     * {@link ChaosActivationHandle#close()}. Terminal state — cannot be restarted.
     */
    STOPPED,

    /**
     * The scenario failed to activate due to a configuration or runtime error. See
     * {@link ActivationFailure} records in the {@link Snapshot} for details.
     */
    FAILED,
  }
   ...
   * @param runtimeDetails capability map: {@code jdkFeatureVersion}, {@code virtualThreadsSupported},
   *                       {@code jfrSupported}, {@code currentSessionId}
   */
  record Snapshot(
      Instant capturedAt,
   ...
   * @param scopeKey      internal scope key: {@code "jvm"} for JVM-scoped, {@code "session:<id>"}
   *                      for session-scoped
   ...
   * @param appliedCount  number of times the effect was actually applied; always
   *                      {@code <= matchedCount}
   * @param reason        human-readable explanation of the current state (e.g.,
   *                      {@code "max applications reached"})
   */
